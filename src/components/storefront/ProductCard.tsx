    ...
    <div className="group bg-card border border-border rounded-md overflow-hidden hover:shadow-editorial transition-all flex flex-col">
      <Link to={`/produs/${p.slug}`} className="relative block aspect-square overflow-hidden bg-muted">
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-1">
          ...
          {p.badge_bestseller && <span className="px-2 py-0.5 bg-amber-500 text-white text-[10px] font-bold rounded-sm">TOP</span>}
        </div>
        <button
          onClick={(e) => { e.preventDefault(); toggle?.(p.id); }}
          aria-label="Favorite"
          className="absolute top-2 right-2 p-1.5 bg-card/90 rounded-full hover:bg-card transition-colors"
        >
      ...
      <div className="p-3 flex-1 flex flex-col gap-2">
        ...
        {(p.rating ?? 0) > 0 && (
          <div className="flex items-center gap-1 text-[11px] text-muted-foreground">
            <span className="text-amber-500">★</span> {Number(p.rating).toFixed(1)} ({p.review_count ?? 0})
          </div>
        )}
        <div className="mt-auto flex items-end justify-between gap-2">
          <div>
            ...
            <div className="text-base font-bold" style={{ color: "#FF3300" }}>{Number(p.price).toFixed(2)} lei</div>
          </div>
          <button
            disabled={outOfStock}
            onClick={() => addItem({ product_id: p.id, name: p.name, slug: p.slug, image_url: p.image_url, price: Number(p.price) })}
            className="p-2 bg-primary text-primary-foreground rounded-sm hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed transition-opacity"
