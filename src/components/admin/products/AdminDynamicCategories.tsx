...
import { Plus, Pencil, Trash2, Zap, X, GripVertical, RefreshCw, Eye, Package } from "lucide-react";
...
const RULE_TYPES = [
  ...
  { value: "tag", label: "Tag conține" },
  { value: "in_stock", label: "În stoc" },
  { value: "low_stock", label: "Stoc limitat" },
  { value: "has_discount", label: "Are reducere activă" },
  { value: "category", label: "Categorie statică" },
  { value: "min_rating", label: "Rating minim" },
  { value: "created_recent", label: "Creat în ultimele X zile" },
  { value: "featured", label: "Produs recomandat" },
];

const emptyForm = {
  name: "", slug: "", description: "", image_url: "", icon: "",
  display_order: 0, visible: true, rules: [] as Rule[],
};
...
export default function AdminDynamicCategories() {
  ...
  const { data: categories = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("dynamic_categories").select("*").order("display_order");
      if (error) throw error;
      return (data || []) as unknown as DynamicCategory[];
    },
  });

  // Product counts per dynamic category
  const { data: productCounts = {} } = useQuery({
    queryKey: ["admin-dynamic-cat-counts", categories.map(c => c.id).join(",")],
    queryFn: async () => {
      const counts: Record<string, number> = {};
      await Promise.all(
        categories.map(async (cat) => {
          const { data } = await supabase.rpc("count_dynamic_category_products", {
            p_rules: cat.rules as any,
          });
          counts[cat.id] = typeof data === "number" ? data : 0;
        })
      );
      return counts;
    },
  ...
  const upsertMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["admin-dynamic-cat-counts"] });
  ...
  const closeDialog = () => { setDialogOpen(false); setEditingId(null); setForm(emptyForm); setPreviewCount(null); };
  const openNew = () => { setEditingId(null); setForm(emptyForm); setPreviewCount(null); setDialogOpen(true); };
  const openEdit = (cat: DynamicCategory) => {
    setEditingId(cat.id);
    setForm({
      name: cat.name, slug: cat.slug, description: cat.description || "",
      image_url: cat.image_url || "", icon: cat.icon || "",
  ...
  const addRule = () => setForm(p => ({ ...p, rules: [...p.rules, { type: "brand", value: "" }] }));
  const updateRule = (i: number, updates: Partial<Rule>) => setForm(p => ({ ...p, rules: p.rules.map((r, idx) => idx === i ? { ...r, ...updates } : r) }));
  const removeRule = (i: number) => setForm(p => ({ ...p, rules: p.rules.filter((_, idx) => idx !== i) }));

  const handlePreview = async () => {
    if (form.rules.length === 0) { toast.error("Adaugă cel puțin o regulă."); return; }
    setPreviewing(true);
    try {
      const { data, error } = await supabase.rpc("count_dynamic_category_products", { p_rules: form.rules as any });
      if (error) throw error;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name.trim() || !form.slug.trim()) { toast.error("Numele și slug-ul sunt obligatorii."); return; }
    if (form.rules.length === 0) { toast.error("Adaugă cel puțin o regulă."); return; }
  ...
  const ruleLabel = (rule: Rule) => {
    switch (rule.type) {
      case "price_range": return `Preț: ${rule.min}–${rule.max} RON`;
      case "brand": return `Brand: ${rule.value}`;
      case "tag": return `Tag: ${rule.value}`;
      case "in_stock": return rule.value === "true" ? "În stoc" : "Fără stoc";
      case "low_stock": return "Stoc limitat";
      case "has_discount": return rule.value === "true" ? "Cu reducere" : "Fără reducere";
      case "category": { const cat = staticCategories.find(c => c.id === rule.value); return `Cat: ${cat?.name || "?"}`; }
      case "min_rating": return `Rating ≥ ${rule.value}`;
      case "created_recent": return `Ultimele ${rule.value} zile`;
      case "featured": return rule.value === "true" ? "Recomandat" : "Ne-recomandat";
    ...
    <div className="space-y-4">
        ...
        <div className="border rounded-lg overflow-hidden">
          <Table>
            ...
            <TableBody>
              {filtered.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center text-muted-foreground py-8">Nicio categorie smart. Creează una!</TableCell>
                </TableRow>
              ) : (
                filtered.map(cat => (
                  <TableRow key={cat.id}>
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-1">
                        {(Array.isArray(cat.rules) ? cat.rules : []).map((rule, i) => (
                          <Badge key={i} variant="outline" className="text-xs">{ruleLabel(rule)}</Badge>
                        ))}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="font-medium">{productCounts[cat.id] ?? "—"}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-1 text-xs text-muted-foreground">
                        <RefreshCw className="w-3 h-3" />
                        {timeAgo(cat.updated_at)}
                      </div>
                    </TableCell>
                    <TableCell>
                      <Badge variant={cat.visible ? "default" : "outline"}>{cat.visible ? "Da" : "Nu"}</Badge>
                    </TableCell>
                    <TableCell className="text-right">
                      <Button variant="ghost" size="icon" onClick={() => openEdit(cat)}><Pencil className="w-4 h-4" /></Button>
                      <Button variant="ghost" size="icon" className="text-destructive" onClick={() => { if (confirm(`Ștergi "${cat.name}"?`)) deleteMutation.mutate(cat.id); }}>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Nume</Label><Input value={form.name} onChange={e => handleNameChange(e.target.value)} required /></div>
              <div><Label>Slug</Label><Input value={form.slug} onChange={e => setForm(p => ({ ...p, slug: e.target.value }))} required /></div>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label className="text-base font-semibold">Reguli (AND — toate condițiile trebuie îndeplinite)</Label>
                ...
                <Card key={idx}>
                  <CardContent className="p-3 flex items-start gap-3">
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      {(rule.type === "in_stock" || rule.type === "has_discount" || rule.type === "featured") && (
                      ...
                      {rule.type === "low_stock" && (
                        <p className="text-xs text-muted-foreground">Produse cu stoc &gt; 0 și ≤ low_stock_threshold</p>
                      )}
                      {rule.type === "category" && (
                        <Select value={rule.value || ""} onValueChange={v => updateRule(idx, { value: v })}>
                          <SelectTrigger><SelectValue placeholder="Alege categorie" /></SelectTrigger>
                          <SelectContent>{staticCategories.map(c => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent>
                        </Select>
                      )}
                      ...
                      {rule.type === "created_recent" && (
                        <Input type="number" min={1} placeholder="Număr de zile" value={rule.value || ""} onChange={e => updateRule(idx, { value: e.target.value })} />
                      )}
                    </div>
              ...
              {form.rules.length === 0 && (
                <p className="text-sm text-muted-foreground text-center py-4 border border-dashed rounded-lg">Nicio regulă adăugată.</p>
              )}
            </div>
            ...
            {form.rules.length > 0 && (
              <div className="flex items-center gap-3 p-3 bg-muted/50 rounded-lg">
                <Button type="button" variant="outline" size="sm" onClick={handlePreview} disabled={previewing}>
                  <Eye className="w-4 h-4 mr-1" /> {previewing ? "Se calculează..." : "Previzualizare"}
                </Button>
                {previewCount !== null && (
                  <span className="text-sm font-medium">
                    <Package className="w-4 h-4 inline mr-1" />
                    {previewCount} produse se potrivesc regulilor
                  </span>
                )}
              </div>
            )}
      ...
      {categories.length === 0 && !isLoading && (
        <div className="border border-dashed rounded-lg p-4 space-y-2">
          <p className="text-sm font-semibold text-foreground">💡 Exemple de categorii smart:</p>
          <ul className="text-sm text-muted-foreground space-y-1 ml-4 list-disc">
            <li><strong>Reduceri</strong> → regulă: „Are reducere activă = Da"</li>
            <li><strong>Noutăți</strong> → regulă: „Creat în ultimele 30 zile"</li>
            <li><strong>Sub 100 lei</strong> → regulă: „Preț între 0 și 100"</li>
            <li><strong>Stoc limitat</strong> → regulă: „Stoc limitat"</li>
          </ul>
        </div>
      )}
    </div>
