...
import {
  RotateCcw, Package, ChevronRight, Clock, CheckCircle2, XCircle,
  AlertTriangle, ShieldCheck, Truck, CreditCard, Search, Info, ArrowRight,
} from "lucide-react";
...
export default function Returns() {
  ...
  // Fetch orders eligible for return (delivered within last 30 days)
  ...
  // Check which orders already have returns
  const ordersWithReturns = new Set(
    (returns || []).map((r: any) => r.order_id)
  );

  const eligibleOrders = (orders || []).filter(
    (o: any) => !ordersWithReturns.has(o.id)
  );
              ...
              {ordersLoading ? (
                ...
                <div className="space-y-2">
                  {eligibleOrders.map((order: any) => {
                    ...
                    const totalValue = order.total || order.order_items?.reduce((s: number, i: any) => s + (i.unit_price || i.price || 0) * (i.quantity || 1), 0) || 0;
                      ...
                      >
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-semibold">Comanda #{order.id?.slice(0, 8)}</p>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-muted-foreground mb-3">
                    Comenzile livrate în ultimele 14 zile sunt eligibile.
                  </p>
