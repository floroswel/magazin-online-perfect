...
export default function CmsPage({ overrideSlug }: { overrideSlug?: string }) {
  ...
  usePageSeo({ title: page ? `${page.title} | LUMAX` : "LUMAX", description: (page as any)?.meta_description || "" });
  ...
  return (
    <Layout>
      <div className="lumax-container py-8 max-w-3xl mx-auto">
        <h1 className="text-2xl font-extrabold text-foreground mb-6">{page.title}</h1>
        <div className="prose prose-sm max-w-none text-foreground" dangerouslySetInnerHTML={{ __html: (page as any).body_html || (page as any).content || "" }} />
      </div>
    </Layout>
  );
}